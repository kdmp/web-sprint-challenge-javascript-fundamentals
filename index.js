...
/* 
In javascript, scopes have hierarchies. The global scope is visible by all other scopes. Visually, these hierarchies would be represented by
drawing circles within circles.

The global scope, where ${external} is defined is visible by all other scopes - the outermost circle.

The function ${myFunction} declare a scope within that global scope. The declaration of this function, which is just a variable of type function,
means that it is now visible in the global scope, same as ${external}. This is visually intuitive by the fact that they are declare alongside
one-another.

Inside the ${myFunction} function scope, a local variable ${internal} is declared. This function is "local" to the function scope.

Inside the ${myFunction} function scope, alongside ${internal}, a function ${nestedFunction} is declared. Inside the scope of ${nestedFunction},
we have access to the following variables: ${external}, ${myFunction}, ${internal} and ${nestedFunction}. This is because all of these variables
were declared along the hierarchy. Visualize this as circles drawn within circles: a scope (depicted as a circle) can see out into the scope of
the circle that it is drawn inside of.
*/
...
function lowerCaseNames(zooAnimals) {
  return zooAnimals.map((a) => a.animal_name.toLowerCase());
}
...
function USApop(zooAnimals) {
  return zooAnimals.reduce(
    (totalPopulation, animal) => totalPopulation + animal.population,
    0
  );
}
